...
class Application():
    ...
    def abrir_janela_cadastro(self):
        ...
        self.buscar_entry = Entry(self.frame_2)
        self.buscar_entry.place(relx=0.05, rely=0.3, relwidth=0.5, relheight=0.034)
        ...
        # Botão Limpar
        self.bt_limpar = Button(self.frame_2, text="Limpar", background='white', bd=3, command=self.limpar_campos)
        self.bt_limpar.place(relx=0.3, rely=0.6, relwidth=0.1)

        # Botão Salvar
        self.bt_salvar = Button(self.frame_2, text="Salvar", background='white', bd=3, command=self.criar_banco)
    ...
    def criar_banco(self):
        ...
        conexao.execute("CREATE TABLE IF NOT EXISTS funcionarios (nome TEXT, email TEXT, setor TEXT)")
        conexao.close()
        ...
        conexao = sqlite3.connect("funcionarios.db")
        conexao.execute(f"INSERT INTO funcionarios (nome, email, setor) VALUES ('{nome}', '{email}', '{setor}')")
        conexao.commit()
        conexao.close()
